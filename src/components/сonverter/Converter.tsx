...
const changeFirstPopupCurrency = (currency: string) => {
  ...
  if (secondPopupCurrency && !(currency === 'USD' || currency === 'EUR' || currency === 'RUR' || currency === 'JPY')) {
    dispatch(setPopupCurrency({ firstPopupCurrencyCharCode: currency, secondPopupCurrencyCharCode: secondPopupCurrency.CharCode }));
  }
...
const changeSecondPopupCurrency = (currency: string) => {
  ...
  if (firstPopupCurrency && !(currency === 'USD' || currency === 'EUR' || currency === 'RUR' || currency === 'JPY')) {
    dispatch(setPopupCurrency({ firstPopupCurrencyCharCode: firstPopupCurrency.CharCode, secondPopupCurrencyCharCode: currency }));
  }
...
const swapCurrencies = () => {
  ...
  dispatch(setPopupCurrency({ firstPopupCurrencyCharCode: secondPopupCurrency.CharCode, secondPopupCurrencyCharCode: firstPopupCurrency.CharCode }))
  }
}
    ...
    <>
      ...
      <div className="converter">
        <div className="converter__title">У меня есть</div>
          ...
          />
          <div className="box__rate">1 {currencyFirstField} = {rateForFirstField} {currencySecondField}</div>
      </div>
      ...
      <div className="converter">
        ...
        />
          <div className="box__rate">1 {currencySecondField} = {rateForSecondField} {currencyFirstField}</div>
      </div>
      {
        isVisible.list && ( <PopupCurrencies
                popupRef={popupRef}
