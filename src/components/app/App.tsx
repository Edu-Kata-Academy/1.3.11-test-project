...
function App() {
  ...
  useEffect(() => {
    if (response && response.Valute) {
      ...
      const mainCurrencies = [response.Valute["USD"], response.Valute["EUR"], response.Valute["JPY"]].filter(Boolean);
  ...
  const { currencies, currencyFirstField, currencySecondField } = useSelector((state: RootState) => state.converter);
